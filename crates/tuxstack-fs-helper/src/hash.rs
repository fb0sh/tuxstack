//! `hash` command: streamed SHA-256 (never loads the whole file).
...
pub fn run(args: &[String]) -> Result<()> {
    ...
    if flags.algorithm != "sha256" {
        return Err(HelperError::new(
            HelperErrorCode::InvalidArgs,
            format!("unsupported algorithm: {}", flags.algorithm),
        ));
    }
    ...
    emit(&HelperMessage::Hash {
        algorithm: "sha256".into(),
        value: hex(&hasher.finish()),
    });
    Ok(())
}

// ---------------------------------------------------------------------------
// SHA-256 (hand-rolled; the helper keeps dependencies minimal)
// ---------------------------------------------------------------------------
