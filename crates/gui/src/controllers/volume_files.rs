...
use tuxstack_docker_core::{VolumeFileEntry, VolumeFileType, VolumePath};
...
impl VolumeFilesControllerState {
    ...
    pub fn apply_list(
        ...
        generation: u64,
        path: VolumePath,
        mut entries: Vec<VolumeFileEntry>,
        truncated: bool,
    ) -> bool {
    ...
    pub fn visible_entries(&self) -> Vec<&VolumeFileEntry> {
...
fn cmp_name(left: &VolumeFileEntry, right: &VolumeFileEntry) -> Ordering {
    let primary = left
        .name
        .to_ascii_lowercase()
...
fn cmp_modified(left: &VolumeFileEntry, right: &VolumeFileEntry) -> Ordering {
...
fn cmp_size(left: &VolumeFileEntry, right: &VolumeFileEntry) -> Ordering {
...
fn cmp_kind(left: &VolumeFileEntry, right: &VolumeFileEntry) -> Ordering {
    kind_rank(left.entry_type)
        .cmp(&kind_rank(right.entry_type))
        .then_with(|| cmp_name(left, right))
}

fn kind_rank(entry_type: VolumeFileType) -> u8 {
    match entry_type {
        VolumeFileType::Directory => 0,
        VolumeFileType::RegularFile => 1,
        VolumeFileType::SymbolicLink => 2,
        VolumeFileType::Socket => 3,
        VolumeFileType::Fifo => 4,
        VolumeFileType::BlockDevice => 5,
        VolumeFileType::CharacterDevice => 6,
        VolumeFileType::Unknown => 7,
    }
}
...
mod tests {
    ...
    fn entry(name: &str, dir: bool, size: Option<u64>) -> VolumeFileEntry {
        VolumeFileEntry {
            name: name.into(),
            path: VolumePath::parse(&format!("/{name}")).unwrap(),
            entry_type: if dir {
    ...
    fn search_filters_current_directory_only() {
        ...
        assert_eq!(state.visible_entries()[0].name, "readme.txt");
    }
    ...
    fn generation_rejects_stale_list() {
        ...
        assert!(!state.apply_list(generation - 1, VolumePath::root(), vec![], false));
        assert!(state.apply_list(generation, VolumePath::root(), vec![], false));
    ...
    fn begin_volume_starts_session_and_clears_previous_rows() {
        ...
        assert!(state.apply_list(
            list_gen,
            VolumePath::root(),
            vec![entry("old.txt", false, Some(1))],
            false
        ));
    ...
    fn starting_session_to_loading_to_ready() {
        ...
        assert!(state.apply_list(
            list_gen,
            VolumePath::root(),
            vec![entry("file.txt", false, Some(3))],
            false
        ));
        assert_eq!(state.state, VolumeFilesState::Ready);
        assert_eq!(state.visible_entries().len(), 1);
    ...
    fn loading_to_empty_and_error() {
        ...
        let list_gen = state.begin_list();
        assert!(state.apply_list(list_gen, VolumePath::root(), vec![], false));
        assert_eq!(state.state, VolumeFilesState::Empty);
    ...
    fn stale_list_cannot_overwrite_newer_volume() {
        ...
        assert!(!state.apply_list(
            stale,
            VolumePath::root(),
            vec![entry("stale.txt", false, Some(1))],
            false
        ));
        assert!(state.apply_list(
            fresh,
            VolumePath::root(),
            vec![entry("fresh.txt", false, Some(2))],
            false
        ));
        assert_eq!(state.volume_name, "two");
        assert_eq!(state.entries.len(), 1);
        assert_eq!(state.entries[0].name, "fresh.txt");
    }
}
